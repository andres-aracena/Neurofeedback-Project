...
from filters import preprocess_signal, bandpass_sos, envelope
from processing import compute_wavelet, update_plots
...
N_CH = 8
WIN_SEC = 5
UPDATE_MS = 100  # un poco más relajado para evitar lag
...
main, ui = create_ui(N_CH, WIN_SEC)
ch_sel = 0
...
# =========================
def set_channel(idx):
    nonlocal_ch = globals()['ch_sel']
    nonlocal_ch = max(0, min(N_CH-1, idx))
...
def update():
    ...
    # análisis y ploteo
    update_plots(buffers, FS, THETA_BAND, GAMMA_BAND, EPS, ui, t0, ch_sel, WIN_SEC)

...
UPDATE_MS = 50                        # Intervalo de actualización de GUI (ms)
...
# Confirmar canales EEG y forzar 8 si el board devuelve menos
eeg_channels = board.get_eeg_channels(BoardIds.SYNTHETIC_BOARD.value)
if len(eeg_channels) >= N_CH:
    eeg_channels = eeg_channels[:N_CH]
else:
    # si hay menos, replicamos índices hasta llegar a 8 (solo para demo sintética)
